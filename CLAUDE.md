...
## 🎬 P1 新コンセプト「二元の観測装置」（2026-04-23 司さん確定）
スクエア内を **左右 2 パネル** に分割、哲学的二元論を物理的に演出する。

### 過程（フェーズ）
1. **SEPARATION** — パネル分離
   - 左パネル: 白背景 / CMY 3球（シアン・マゼンタ・イエロー）
   - 右パネル: 黒背景 / RGB 3球（赤・緑・青）
2. **FUSE_DUAL** — 各パネル内で 3 色融合
   - 左（白背景 + CMY）→ 減法混色で **黒** になる
   - 右（黒背景 + RGB）→ 加法混色で **白** になる
3. **MERGE_OPPOSITES** — 左の黒と右の白が中央で衝突 → **グレー**（50%）
4. **OBSERVE** — グレー球から放射状に **RGBCMY 虹色トンネル**が広がる（101% への跳躍）
5. **DONE** → `inryoku:p1complete` 発火 → P2 へ

### 視覚キー
- 左白 / 右黒（コントラスト）→ 左右の境界線は観測の境
- CMY は顔料・物質・マット、RGB は光・精神・発光（前の GREY APPARATUS 踏襲）
- 「白 + 黒 = グレー」の瞬間は減法 + 加法の両方が同時に成立する奇跡として演出
- 虹色トンネルは円形放射 + 消失点に向かう 3D 奥行き

### 保持
- Win95 外側窓（タイトル / メニューバー / 進捗バー / タスクバー）
- 音: CMY=Am / RGB=AMaj / 融合で 6音グレーコード
- 自動進行・モバイル P3 直行スキップ
